...
//此函数用于使用对角线数独模块
{
	...
	cout<<"请输入想要挖的洞的数目\n";
	cin>>diagdoku.holes;
    sum_num=N*N-diagdoku.holes;
    diagdoku.remove_digits();
    diagdoku.copy_sudoku();
    // 挖洞后的数独
    cout << "\n生成的数独为:\n";
    diagdoku.print_sudoku(diagdoku.modified_sudoku);
    ...
    // 读取第二行头两个字符
        ...
        {   
            ...
            if(temp1>0)
            {
                diagdoku.inverse_var(temp1,i,j,k);
                ans[i-1][j-1]=k;
                cnt++;
            }
        }   
    // 关闭文件
    	fclose(file2);
        cout << "标准答案:\n";
       diagdoku. print_sudoku(diagdoku.sudoku);
        printf("SAT求解答案：\n");
        diagdoku. print_sudoku(ans);
		printf("程序结束\n");
		break;
    case 3:
        diagdoku.copy_sudoku();
        cout<<"当前数独如下"<<endl;
        diagdoku.print_sudoku(diagdoku.newsudoku);
        cout<<"请输入你想填入数独的位置（i，j），以及填入的数字num（只需输1个0退出填写）"<<endl;
        cin>>row;
        while(row!=0)
        {
            cin>>col>>num;
            diagdoku.newsudoku[row-1][col-1]=num;
            sum_num++;
            cout<<"当前数独如下"<<endl;
            diagdoku.print_sudoku(diagdoku.newsudoku);
            if(num!=diagdoku.sudoku[row-1][col-1])
            cout<<"填写错误！此处应填写 "<<diagdoku.sudoku[row-1][col-1]<<endl;
            else
            cout<<"填写正确！"<<endl;
    ...
    case 4:
            ...
            for(int i=0;i<N;i++)
        {
            for(int j=0;j<N;j++)
            {
                if(diagdoku.newsudoku[i][j]!=diagdoku.sudoku[i][j])
                flag=0;
            }
        }
        if(flag==1)
